...
X_train <- model.matrix(FLAG~. , dt_woe)[,-1]
y_train <- dt_woe$FLAG

X_test = model.matrix(FLAG~. , dt_woe_test)[,-1]
y_test = dt_woe_test$FLAG
...
cv.out <- cv.glmnet(X_train, y_train, family=c("binomial"),alpha = 1 , lambda = lambda , nfolds=10)
...
as.list(small.lambda.betas)


model_fin <- glmnet(X_train, y_train, family=c("binomial"), alpha = 1  , lambda = bestlam )
coef=predict( model_fin , type = "coefficients", s = bestlam)
coef
##coef(mod)
...
train_pred <- predict( model_fin , type='response',   newx = X_train) # type='coef' ,
summary(train_pred)
train_pred = as.double(train_pred)

test_pred <- predict( model_fin ,  type='response',   newx = X_test)
test_pred = as.double(test_pred)
...
# performace
# ks & roc plot
perf_eva(y_train , train_pred, type = c("ks","lift","roc","pr"), show_plot=FALSE)

# ks & roc plot
perf_eva(y_test , test_pred, type = c("ks","lift","roc","pr"), show_plot=FALSE)
...
alpha_beta = alpha_beta(basepointsset,   baseoddsset,  pdo )
alpha=alpha_beta$alpha
beta=alpha_beta$beta


score_train= alpha-beta*log(train_pred/(1-train_pred))
summary(score_train)
# hist(score_train,breaks=14, xlab="score",main="Score Hist Train")





score_test= alpha-beta*log(test_pred/(1-test_pred))
...
score_train_plt= as.data.frame(cbind(score_train, FLAG_train))

# perf_eva(score_train_plt$FLAG, score_train_plt$score, type = c("ks","lift","roc","pr"), show_plot=FALSE)
 
pg =    data.frame(score_train_plt[FLAG_train==0, 'score_train'])
pb =    data.frame(score_train_plt[FLAG_train==1, 'score_train'])
...
## plot test
FLAG_test = test$FLAG
score_test_plt= as.data.frame(cbind(score_test, FLAG_test))

# perf_eva(score_test_plt$FLAG, score_test_plt$score, type = c("ks","lift","roc","pr"), show_plot=FALSE)


pg =   data.frame(score_test_plt[FLAG_test==0, 'score_test'])
pb =   data.frame(score_test_plt[FLAG_test==1, 'score_test'])
...
colnames(score_train_plt)=c('score','FLAG')
colnames(score_test_plt)=c('score','FLAG')


psi_s = perf_psi(
  score = list(train = score_train_plt , test = score_test_plt ),
  label = list(train = y_train, test = y_test),
  title='PSI',
